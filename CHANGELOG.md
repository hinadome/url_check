...
- **Capture HAR** checkbox (default **off**, under Custom headers): Playwright `recordHar` (`mode: "full"`) with two formats — **`harFormat: "zip"`** (default, `content: "attach"` → `session.har.zip` → `harZipBase64` / `.har.zip` download, binaries as zip files) and **`harFormat: "json"`** (`content: "embed"` → `session.har` → `har` / `.har` download, binaries base64-inlined). UI radios under Capture HAR. Ephemeral OS temp during the check only; no app/DB persistence. Soft size cap **`MAX_HAR_BYTES`** (`25_000_000`) on the archive file — over limit sets `harError` and keeps page results. Plan: [`docs/HAR_ZIP_IMPLEMENT_PLAN.md`](docs/HAR_ZIP_IMPLEMENT_PLAN.md)
...
- Form layout: **Ignore certificate errors** and **Capture HAR** sit **under Custom headers** (after the header editor); Capture HAR includes **Zip / JSON** format radios (`harFormat`, default zip).
...
- `README.md` — [Export](README.md#export) / [HAR capture](README.md#har-capture-playwright-session-archive) (dual `harFormat` zip/json, `MAX_HAR_BYTES`, `har` / `harZipBase64`); plan [`docs/HAR_ZIP_IMPLEMENT_PLAN.md`](docs/HAR_ZIP_IMPLEMENT_PLAN.md)
- [`DEPLOYMENT.md`](DEPLOYMENT.md) — Capture HAR formats / `MAX_HAR_BYTES` host impact; feature gates; `NGINX_DISABLE_DEFAULT`; replay script pointer
